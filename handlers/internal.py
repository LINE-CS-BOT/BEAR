...
_STAFF_ORDER_LINE_RE = re.compile(
    r'(.+?)\s+(?:訂|下單)\s+([A-Za-z]{1,3}-?\d{3,6}(?:-\d+)?)\s+([零一二三四五六七八九十百千\d]+)\s*(個|件|盒|套|箱|組)?\s*(.*)'
)
...
_STAFF_ORDER_DIRECT_RE = re.compile(
    r'^(.+?)\s+([A-Za-z]{1,3}-?\d{3,6}(?:-\d+)?)\s+([零一二三四五六七八九十百千\d]+)\s*(個|件|盒|套|箱|組)?\s*(.*?)$'
)
_BULK_UNITS = {"件", "箱"}
...
def handle_spec_inquiry_qty(group_id: str, text: str, line_api) -> str | None:
    ...
    actual_qty, unit = resolve_order_qty(prod_code, qty)
    ...
    if slip_no:
        return f"✅ 已建立訂單 {slip_no}\n  {name}（{prod_code}）× {actual_qty} {unit}"
    else:
...
def _do_order(
    ...
    _phone      = ""
    ec_list     = []   # 初始化，供步驟 2 fallback 使用
    is_new_cust = False

    # 去除括號（「陳怡如(彥鈞)」→「陳怡如」）以利查詢
    cust_name_clean = _re.sub(r'[\(（][^\)）]*[\)）]', '', cust_name_query).strip() or cust_name_query
    ...
    try:
        ...
        if not ec_match and cust_name_clean != cust_name_query:
            ec_match = next((x for x in ec_list if x.get("name", "") == cust_name_clean), None)
        if not ec_match:
            ec_match = next((x for x in ec_list if cust_name_clean in x.get("name", "")), None)
        if ec_match:
    ...
    if not cust_code:
        ...
        if not cust_matches:
            ...
            if user_id:
                ...
                if ec_name:
                    ec_match2 = next((x for x in ec_list if ec_name in x.get("name", "")), None)
                    if ec_match2:
...
def handle_internal_order(
    ...
    def _apply_unit(prod_cd: str, qty: int, unit: str | None) -> int:
        """若單位是箱/件，套用換算；否則原樣回傳"""
        if unit and unit in _BULK_UNITS:
            return _resolve_qty(prod_cd, qty)
        return qty
        ...
        if not _STAFF_ORDER_LINE_RE.search(lines[0]):
            ...
            for l in lines[1:]:
                ...
                if im:
                    ...
                    items_raw.append((prod_cd, _apply_unit(prod_cd, qty, unit)))
    ...
    if len(lines) > 1 and not _PROD_CODE_RE.search(lines[0]) and not _STAFF_ORDER_HEADER_RE.match(lines[0]):
        ...
        for l in lines[1:]:
            ...
            if im:
                ...
                qty     = _parse_qty(im.group(2))
                unit    = im.group(3) if im.lastindex >= 3 else None
                items_b2.append((prod_cd, _apply_unit(prod_cd, qty, unit)))
            elif re.match(r'^備[註誌记]\s*[:：]?\s*', l):
    ...
    if valid:
        ...
        for _line, m in valid:
            ...
            res = _do_order(
                ...
                items_raw=[(_prod_cd, _apply_unit(_prod_cd, _qty, _unit))],
                note=_note_a,
            )
            results.append(res)
        return "\n".join(results)
    ...
    if len(lines) == 1:
        ...
        if m_c:
            ...
            return _do_order(
                ...
                items_raw=[(_prod_cd_c, _apply_unit(_prod_cd_c, _qty_c, _unit_c))],
                note=_note_c,
            )
...
def handle_internal_notify_register(text: str, line_api=None) -> str | None:
    ...
    # ── 格式 A：「登記通知\n第二行\n後續行...」─────────────────────────────
    _PROD_CODE_PAT = re.compile(r'^[A-Za-z]{1,3}-?\d{3,6}(?:-\d+)?$', re.IGNORECASE)
...
# 純貨號偵測（整條訊息只有貨號）
_CODE_ONLY_RE = re.compile(r'^\s*([A-Za-z]{1,3}-?\d{3,6}(?:-\d+)?)\s*$', re.IGNORECASE)
