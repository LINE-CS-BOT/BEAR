...
- 售價「{price}」：商品名稱正下方，字色 #FF6B00 橘色，字重粗，字級「中大」，置中。這是硬性要求，一定要顯示價格。
- 貨號「{prod_code}」：售價正下方，字色 #999999，字級「小」，字母間距略寬，置中
- 各元素之間有 8px 的呼吸空間
...
- 字體統一無襯線黑體，乾淨俐落
- 無多餘裝飾，無漸層色塊，無花俏邊框
- 整體視覺動線：品牌名 → 商品 → 商品名 → 售價 → 貨號 → LINE 聯繫
...
第三層｜售價（必須顯示）：
「{price}」，字色 #FF6B00 橘色，字重粗，字級「中大」。這是硬性要求，一定要顯示價格。
售價下方 10px 留白。
...
- 視覺動線：商品圖 → 品牌名 → 商品名 → 售價 → 貨號 → 賣點 → LINE
...
def handle_ad_update_trigger(text: str, group_id: str,
    ...
    if m:
        ...
        return (
            f"🧪 廣告圖測試：{code}  {prod_name}  售價{price}  {img_note}\n"
            f"生成 LINE + FB 共 2 張\n"
            f"⏳ Gemini 即將開啟，完成後通知"
        )
